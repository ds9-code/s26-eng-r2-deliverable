...
export default function SpeciesCard({ species, userId }: { species: Species; userId: string }) {
  // Seed species that should not be editable, even if they have the user's ID as author
  const seedSpeciesNames = [
    'Cavia porcellus',
  ...
  const isAuthor = species.author === userId && !seedSpeciesNames.includes(species.scientific_name);
  ...
  return (
    <div className="m-4 w-72 min-w-72 flex-none rounded border-2 p-3 shadow">
      ...
      <div className="mt-3 flex gap-2">
        ...
        {isAuthor && <DeleteSpeciesDialog species={species} />}
      </div>
    </div>
  );
}
